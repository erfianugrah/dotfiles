...
test("loopback provider detected", () => {
  ...
  // alias: llama-server (pre-2026-09-08 name) - both ids match
  expect(isLocalProvider({ provider: "llama-server" })).toBe(true);
  expect(isLocalProvider({ provider: "llmc" })).toBe(true);
});
...
test("llmc hint names the stack and the recovery command", () => {
  // alias: llama-server (pre-2026-09-08 name) - both ids trigger the hint
  const h1 = buildHint("Connection error.", { provider: "llama-server", baseUrl: "http://localhost:11434/v1" });
  expect(h1).toContain("llmc");
  expect(h1).toContain("make up");
  expect(h1).toContain("retrying will not fix it");
  const h2 = buildHint("Connection error.", { provider: "llmc", baseUrl: "http://localhost:11434/v1" });
  expect(h2).toContain("llmc");
  expect(h2).toContain("make up");
  expect(h2).toContain("retrying will not fix it");
});
