...
| Compose stack patterns + bridge networks + Caddy + static IPs | `infrastructure-stack` |
...
```dockerfile
# syntax=docker/dockerfile:1                # track the latest stable Dockerfile frontend
ARG GO_VERSION=<pin-current-go>
...
3. **`.dockerignore` is mandatory** - ship one with every project. At minimum: `.git`, `node_modules`, `dist`, `*.log`, `.env*`, `target/`, `__pycache__`, your repo's `tmp/` equivalent. Saves 10-100x context-send size.
...
# build cache
docker buildx prune --max-used-space=10GB               # cap build cache at 10GB (buildx v0.36)
...
- **Build is slow but cache is hot** -> profile with `docker buildx build --progress=plain` and look at "CACHED" vs "DONE" lines.
