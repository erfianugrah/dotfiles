...
description: "Use when writing, reviewing, or tuning Postgres SQL, indexes, RLS policy predicates, schema types, locking, or pooler config - on Supabase or any Postgres. Fires on 'slow query', 'which index', 'EXPLAIN ANALYZE', 'RLS policy is slow', 'pg_cron cleanup', 'SKIP LOCKED', 'keyset pagination', 'TOAST', 'autovacuum'. NOT for platform/API/CLI/auth work (supabase), a performance report (pg-analyser), or migrations (sbshift)."
license: MIT
metadata:
  author: supabase
  version: "1.4.0"
...
Performance and correctness rules for Postgres, prioritised by impact. SQL-side
only: client-side handling of PostgREST errors, Realtime CSP and the supabase-js
wiring live in the `supabase` skill; this skill is what the database should
look like.
...
- **Cap string inputs that feed `tsvector` / GIN indexes.** Unbounded `text` columns indexed by GIN bloat the index quickly. Enforce limits at the app layer (Zod `.max(N)`, see `supabase`) and/or with a CHECK constraint.
...
- **UPDATE needs a SELECT policy**; without one the UPDATE matches zero rows and reports success. A DELETE blocked by RLS also reports success with zero rows - the client-side detection (`count: 'exact'`) is in `supabase`.
...
- **Pre-check-then-insert is a TOCTOU race.** Let the unique constraint be the arbiter: the second writer gets `23505` (unique_violation). The app layer translates that to a typed HTTP 409 (`supabase` skill has the client side); without the constraint there is nothing to catch.
...
- A `.single()` that matches zero rows is a normal outcome, not a database error; PostgREST reports it as `PGRST116` and the client short-circuit is in `supabase`.
...
- **Benchmark before/after with `pgbench`** to prove a fix under concurrency (not just in EXPLAIN): custom `-f` scripts, skewed `random_zipfian` for hot rows, `-l` logs for p95/p99, extended protocol through Supavisor. Tested guide: `~/work/pg-analyser/docs/pgbench.md`; `pg-analyser bench` wraps it with guardrails.
...
- **Verify the subscriber exists end-to-end before installing a Realtime trigger.** A trigger that calls `realtime.send(...)` on every insert with no consumer burns the Realtime message quota (Free: 2M/mo) for nothing. Two ways this happens: the frontend never wires `.channel(...).subscribe(...)`, or its CSP blocks the WebSocket (the CSP check is in `supabase`, `reference/realtime.md`).
...
## Settling a semantics question

When two confident answers disagree about what Postgres does (RLS recursion
shapes, what resets `pg_stat` counters, TOAST accounting), reproduce the exact
shape in a throwaway `postgres:17-alpine` container and read the docs page for
the claim before encoding it in a rule or a report. Tear the container down
afterwards.
...
- Docs mirror topic `postgres` (pi `docs_search`/`docs_read`; Claude Code erfi-toolkit `docs` action=search|read) - full Postgres reference offline.
- `supabase` skill - platform, client-side error handling, pooler hosts.
- `pg-analyser` skill - turns these rules into a ranked report for a live database.
