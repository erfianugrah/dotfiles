...
11. **Cross-stack shared networks** for services that need to consume across boundaries (e.g. `media` network for jellyfin consumers). **A network that two independently deployed stacks both need is HOST-owned, not owned by whichever stack happens to declare it**: the host creates it (on servarr, a oneshot in `servarr-nixos/modules/docker.nix`) and every stack, producer included, declares it `external: true`. See the deploy-order pitfall below for what stack ownership costs.
...
- **Port-publishing on an `internal: true` network silently does nothing.** Docker creates NO DNAT rule for it - runtime `NetworkSettings.Ports` stays `null` and `docker port` is empty, with no error. If another stack needs to scrape/reach a service on an internal network, the consumer must be a MEMBER of a network the service is on; never try to publish a host port. For a ONE-OFF, join the producer's network as `external: true` in the consumer. For a PERMANENT coupling, do not: put both on a host-owned link network instead (`--internal` keeps the isolation), so neither stack owns the other's dependency. monitoring's prometheus joining `memledger_backend` to scrape the postgres exporter is the one-off form, and it is what made monitoring undeployable whenever memledger had not deployed first.
- **An `external: true` network is a DEPLOY-ORDER dependency, and Compose will not create it.** The consumer fails with `network <name> declared as external, but could not be found` until whoever owns it has deployed. Composer's bulk deploy runs stacks in parallel with no inter-stack ordering, so "deploy everything" fails for whichever consumer wins the race. On 2026-09-07 a lost docker network store turned this into 18 failed stacks at once. Three defences, in order of preference: host-own the network (removes the dependency); declare it (`PUT /api/v1/stacks/{name}/depends-on`, honoured by `POST /api/v1/stacks/deploy-batch`); or encode it as `depends_on` between `compose_up` steps in a composer pipeline. Never rely on remembering the order.
...
- **Assigning a static/macvlan IP without checking it's free.** On a shared macvlan (e.g. `servarr_lan`) a colliding `ipv4_address` fails at `up` time with `Error response from daemon: failed to set up container networking: Address already in use`, and the partial `up` leaves the stack HALF-DOWN (some containers stopped by the preceding `down`, the new one never started). Recover by `docker start`-ing the stopped peers, then redeploy on a free IP. Verify free BEFORE assigning: `arping`/`ping` the candidate from a host ON that L2 segment (the router, for `servarr_lan`) - do NOT assume "between two neighbours" is free.
...
- **Skipping health checks**: Compose can't sequence `depends_on: condition: service_healthy` without them. Always declare a healthcheck - EXCEPT for a distroless image (no shell, no `wget`/`curl`: e.g. `grafana/loki`, many `gcr.io/distroless/*`). An in-container `test: ["CMD","wget",...]` there can NEVER run, so the container sits permanently `unhealthy` while the service is fine. Omit the healthcheck (only safe if nothing `depends_on` it) and probe its `/ready` from a LAN host, or use a variant image that ships a client.
