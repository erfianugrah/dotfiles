...
4. **Present-participle padding.** Trailing "-ing" clauses that add verdict, not content: "..., highlighting its importance", "..., ensuring a seamless experience", "..., underscoring the need for". If the clause only grades what you just said, delete it (same family as corpo tell #7). The sneakiest form is the TAUTOLOGICAL tail - the participle restates the fact as if it were a consequence: "the stuntwoman switch makes it look like one continuous shot" when the shot BEING continuous is what makes it look continuous. Test: delete the clause; if the sentence loses no information, the clause was decoration.
...
6. **The slop watchlist.** High-signal AI vocabulary that is never Erfi's: "delve", "tapestry", "leverage", "utilize", "foster", "underscore", "testament", "realm", "landscape" (metaphorical), "pivotal", "vibrant", "seamless", "robust", "cutting-edge", "game-changer". Each model also has pet words a reader learns to hear (Language Jones (Dr Taylor Jones, PhD linguistics), "How to spot LLM writing beyond the obvious", YouTube, 2026): Claude reaches for "toolkit", "the move" as a verdict noun ("X is the move", "the rhetorical move"), "what it buys you", "point X at Y" in the figurative sense, and "the catch"; Gemini for figurative "bucket(s)", "gap", "blueprint", "overarching", "undergird", "pillar"; all of them for "quietly", "amid", "nestled", "honestly", "actually", and the self-validating tag "and that matters". Literal uses stay ("an S3 bucket", "the NVIDIA container toolkit", "point the CNAME at the tunnel"); the figurative ones go. These leaked into the blog corpus through model first drafts (grep of `/docs/erfi-technical-blog` on 2026-09-14: "quietly" x7, "the catch is" x3, "buys you" x1), so a blog hit is drafting residue, not licence.
7. **Mystery-tease framing (engagement-bait reveal).** "hides a classic movie trick", "what they don't tell you", "the secret:", "you may not know", "a little-known" - constructing the sentence as a reveal to be unpacked instead of just stating the mechanism. This is Reddit/LinkedIn title voice: the information is withheld one beat to manufacture curiosity. State the mechanism in the main clause and the curiosity evaporates on its own.
...
10. **Metaphor incoherence - words that co-occur, images that do not compose.** The highest-signal tell of all, and the one the vocabulary lists miss (Language Jones (Dr Taylor Jones, PhD linguistics), "How to spot LLM writing beyond the obvious", YouTube, 2026). A next-token model picks words by co-occurrence, so it produces phrases whose pieces each belong with something while the whole picture is impossible: "nestled amid a year of war" (nestled is cosy, war is not), "two overarching pillars that undergird" (a pillar cannot both arch over and prop up), "words aren't empty containers, a blank slate to be filled" (you fill a container and write on a slate; two senses of fill collide in one clause), "see what the toolkit buys you when you point it at a problem" (tools are neither currency nor aimed). A reader who notices one of these stops trusting the rest of the text. Test before shipping: literalise every figurative phrase and picture it. If the picture is impossible, or two images share one clause, replace the figure with the literal fact ("the deployment came in the middle of a year of war"; "the two categories the rest of the doc depends on"). Erfi's default is the literal statement anyway (characteristic 10), so the fix is usually shorter than the figure. Sweep: `rg -in 'nestled|\bamid\b|quietly|overarching|undergird|pillar|toolkit|buys you|point (it|them|that) at|the move\b|the catch\b|honestly|that matters'` and read each hit literally.
11. **Ad-copy fragments and self-validation.** A fragment used as punch ("Honestly." / "The catch." / "And that matters.") is corpo tell #2's colon-headline with the colon removed, and "you're not being irrational, you're responding to X" / "that's macro-level thinking" is the model validating the reader or grading its own argument (corpo tell #7). Rejoin the fragment to its sentence with a connector; delete the validation.

The `ai-tell-guard` hook (pi extension and Claude Code PreToolUse hook, same core) hard-blocks only the highest-precision subset of this list in prose-file writes and commit messages: the "not just X, but Y" / "isn't about X, it's about Y" / "It's not X. It's Y." forms of #1, the mystery-tease phrases of #7, the "No X, no Y. Just Z." aphorism of #8, importance-announcing ("worth noting", "worth mentioning", "important to note"), and `delve` / `tapestry` / `game-changer` / "stands as a testament" from #6, plus `nestled` / `undergird` (added 2026-09-14 for tell #10; no honest technical use) and "and that matters" under importance-announcing (tell #11). Everything else here - triplets, participle tails, decorative bold, metaphor coherence itself, the softer watchlist words and the model pet words - is judgement, not a gate. Quoted examples (double quotes or backticks) pass the guard in files; in a commit message there is no quoting escape.

Three guards against overcorrection. First, the generic "humanize" prompts go much further -- banning semicolons, dashes, "however / while / although / because", and mandating 10-20-word sentences. Do NOT import those: connectors, subordination, and the `;`-flow ARE the voice (disposition #2, characteristic 12), and a sentence-length metronome reads as more machine-made, not less. Second, the goal is sounding like Erfi, not fooling a detector -- a lone flagged word is not a crime. Kill clusters of tells; never flatten the voice to dodge a wordlist. Third, a lone device is not evidence: an honest contrast ("it's not AI drivel, it's classical rhetoric", in the linguist's own words) is the They Say / I Say move academic writing teaches, and parentheticals, dash-asides and groups of three are ordinary rhetoric. What marks a text as generated is the cluster - several shape tells in one paragraph plus a metaphor that does not literalise (tell #10) - so judge the paragraph, not the phrase, and keep a triplet that is true.
...
| Figurative phrase that does not literalise (a toolkit that buys, a pillar that overarches) | Picture it; if the picture is impossible, write the literal fact (structural tell #10). |
