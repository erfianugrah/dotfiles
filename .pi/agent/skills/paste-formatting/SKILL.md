...
description: Use when text drafted in Markdown has to land in a destination that does not render raw Markdown - Gmail / Outlook compose, Google Docs, Notion, a CMS/WYSIWYG box, an issue tracker, OR a chat app (Slack, Discord, Telegram, WhatsApp). Fires on 'paste into email', 'copy to clipboard for Telegram/Discord/Slack/WhatsApp', 'the formatting got stripped', 'bullets/bold/code came out as raw asterisks or plain text', 'code snippet looks janky in the email', or any copy-paste-loses-formatting gripe. Pairs with erfi-voice (which drafts the prose) - this skill gets that prose into the target intact.
---
