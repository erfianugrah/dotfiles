...
description: "Use when designing, building, or operating an anti-abuse or fraud-detection system - defending a free tier, signup flow, public form, upload endpoint, API, or code-execution platform against spam, phishing, cryptomining, multi-account farming, or fake reviews. Fires on risk scoring, indicator/signal design, blacklists, account bans, false-positive handling, actor tracking, campaign/pivot detection, 'is this abusive and what do we do'. NOT for patching the vulnerability itself (appsec/eng)."
---
...
The fraud-detection project (`~/cf-stuff/fraud-detection/forminator`) is the canonical local instantiation - its `src/lib/scoring.ts` is the per-submission scorer and `src/lib/footprint.ts` the campaign matcher; this skill is the operating model around them.
...
## The three facets of abuse (easy to hard for the attacker)
...
The difficulty ladder **only holds if** systems are reasonably hardened and custom code follows a security-aware SDLC. If exploitation is trivially easy, the ladder collapses.
...
- **Splash** - multiple partial footprints at once = multiple actors sharing TTPs (same forum kit / video tutorial). Find the *source* and pre-empt.
- **Retool** - actor rebuilds everything -> indistinguishable from a new actor. **So what?** The only reason to link old to new is law enforcement evidence (rare). Let retooled actors be new actors; thorough indicator collection re-catches them anyway.
...
5. **Banned actors must not return** - track return indicators even with no fresh abuse. Implication: you **cannot fully delete** actor data - it's needed to detect return (tension with #8).
...
- `footprint.ts` - zero-dependency reference: `matchFootprint`, `matchAll`, `detectSplash`, `suggestFootprintUpdate`, `applyFootprintUpdate`. Framework-agnostic; adapt into any scorer. Forminator's copy lives at `~/cf-stuff/fraud-detection/forminator/src/lib/footprint.ts` with `tests/footprint.spec.ts`.
