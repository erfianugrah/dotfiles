...
- **Router-host stacks have NO GitHub webhook** (e.g. `knotea` on the MS-01 router), so a push to `main` does nothing until something calls `POST /api/v1/stacks/<name>/deploy` - the knotea release workflow does exactly that and polls the stack's `last_commit` (2026-09-18: its `last_sync` had sat four days stale). Check `GET /api/v1/webhooks` before assuming a stack auto-deploys.
