...
- **NEVER amend + force-push a commit composer has already pulled.** The checkout goes non-fast-forward and every `sync` 500s with `pulling: non-fast-forward update`. Fix: `docker exec -u composer -e HOME=/home/composer composer git -C /opt/stacks/<name> reset --hard origin/main` (plain exec reads root's ssh config; composer's own git is go-git with keys from `/home/composer/.ssh`). Hit 2026-08-29 on edge-services.
