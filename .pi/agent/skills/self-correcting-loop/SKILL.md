...
| `loop-interrupt.integration.test.ts` | The loop takes its agent down with it: SIGINT/SIGHUP/SIGTERM to the loop kill the agent's process group, the report lands as `interrupted` naming the in-flight iteration, the run journals with the `interrupted` tag; and while the agent runs the report already says `running` (no stale verdict). |
| `loop-deadman.integration.test.ts` | bwrap is the loop's direct child: a SIGKILLed loop still takes the jailed agent down (`--die-with-parent`), `loop report` flags the dead loop pid, and GNU timeout inside the jail still classifies a hung agent as a timeout. |
