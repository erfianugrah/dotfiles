...
9. **Workers Static Assets serve HTML from a per-PoP cache, ignoring your cache-control.** A static-assets deploy (wrangler `assets` block, no worker script) emits `cf-cache-status: HIT` on HTML even at `max-age=0, must-revalidate`, and a client `no-cache` request still gets the HIT - one PoP held the previous deploy for 24+ minutes while cache-busted URLs were already fresh (erfi.dev, 2026-08-17). Fix: purge the zone after every deploy (`POST /zones/{id}/purge_cache` with `{"purge_everything":true}`), wired into BOTH the local deploy script and CI - a `wrangler-action` `command: deploy` step bypasses package.json hooks, and the deploy token needs the Zone:Cache Purge permission or the purge 401s.
