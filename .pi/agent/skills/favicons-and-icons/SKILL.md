...
description: Use when the user wants a favicon, app icon, brand mark, or full PWA icon set (favicon.ico, apple-touch, 192/512/maskable, site.webmanifest). Fires on 'favicon', 'app icon', 'PWA icons', 'apple-touch-icon', 'site.webmanifest', 'logo or brand mark for a site'. NOT for Open Graph / social preview images or app-store artwork.
---
...
- **Don't hand-roll the HTML snippet.** Paste the snippet the tool returns - it lists every file it generated, in the right order, so it cannot drift from the output set.

## When NOT to use

- **Open Graph / Twitter card / social preview images** - those are 1200x630-class raster images with text, a different pipeline; the favicon set is square marks only.
- **App-store artwork** (iOS App Store, Play Store listings) - store-specific sizes and no-transparency rules; produce those from the 1024 master separately.
- **Print / vector brand assets** (letterhead, signage) - keep the SVG source, but this tool only emits web icon sizes.
