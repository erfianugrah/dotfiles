...
`sessions.project` is `basename(startup cwd)` for every ingester, frozen at session start - pi's jsonl header cwd is never updated on cd. Sessions run from a container dir (`~/infra`) or the wrong repo are filed under THAT project even when all their work touched another (measured: 27 sessions mentioned `hearth` in content, only 5 had `project='hearth'`). Tool-call args carry absolute paths into `messages.content`, so message FTS is the retroactive topic signal.
...
- **History-first is process, not fallback**: before starting ANY non-trivial task, one `memledger_search` / `search_messages` with 2-3 terms from the task (component name, error text). The `history-first` extension (pi) / UserPromptSubmit hook (CC) enforces the reminder until a lookup fires.
...
curl -s "https://memledger.erfi.io/rpc/search_messages?q=X&src=claude" | jq     # per-client filter (pi|claude|opencode)
