...
`~/infra/arrctl` (Go, stdlib-only, static) encodes the arr operations as tested commands: `import` (the verified `GET /manualimport -> POST ManualImport -> Rename*` flow with the command outcome checked, so "imported 0" is a non-zero exit), `naming set` (TRaSH naming with apply-and-verify, handles the Sonarr nested-audiocodec render bug), `export` / `restore` (0600 config dumps; restore recreates into a fresh instance), `restructure` (destructive folder canonicalisation, dry-run by default, `-execute` + inode-aware guards + `-opslog`), `clients pause/resume` (SAB + nzbget), and the 2026-09-14 trim workflow: `owner export` (catalog dump with imdb id + site rating), `rating join` (join an external rating CSV, e.g. Criticker 0-100, by imdb id into the `external` column), `rating tier` (per-band tables), `trim list` (union of `-below`/`-imdb-below`/`-corpus`/`-actor`/`-exclude-anime` into a dry-run manifest), `trim delete` (the *arr "remove with data": editor bulk delete `/movie/editor`+`/series/editor` - the UI's exact body, no `minimumAvailability`; `-files` removes the files, `-exclude` adds an import exclusion so the app doesn't re-add it (radarr `addImportExclusion` / sonarr `addImportListExclusion`); apply-verify, **dry-run default** + `-execute`), and `staging orphan` (hardlink census + freshness guard, staging roots only). It is a flake input of `~/infra/servarr-nixos` and installed on the NAS (`nix flake update arrctl` there to bump); details in `~/infra/arrctl/AGENTS.md`. Reach for it instead of hand-rolled curl.
