...
### Git-over-SSH endpoints + deploy keys (2026-09-19, paid for the hard way)

- **The reachable SSH endpoint is Caddy's L4 proxy `:2223 -> 172.20.6.2:2222`** (router edge). Router :22 is the router's own sshd (no git); :2222 lives only on compose networks and is unreachable from both the dev box and the router host (`Connection refused` / `No route to host`). So git URLs are `ssh://git@<ip>:2223/owner/repo.git` (dev box: git.erfi.io:2223 via ~/.ssh/config; LAN/off-DNS: 10.0.69.1:2223). Forgejo's default host key is RSA - an RSA key on :2223 is Forgejo, not a misdirection.
- **`ssh-keyscan` cannot talk to the forge git sshd** (it closes non-git sessions). Host key for known_hosts: `docker exec forgejo sh -c "cat /var/lib/gitea/ssh/gitea.rsa.pub"`, entry form `[10.0.69.1]:2223 ssh-rsa AAAA...`.
- **Deploy keys** (repo settings, API `POST /repos/<o>/<r>/keys`, title + key + read_only): the clean CI push credential - no PAT, no 90-day expiry. Write key private PEM in a repo secret; the job does `printf '%s\n' "$KEY" > ~/.ssh/k && chmod 600 && GIT_SSH_COMMAND="ssh -i ~/.ssh/k -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new" git push ssh://...`. Read keys for machine checkouts (e.g. composer stacks).
- **IP-pin anything that must survive a LAN resolver outage** (the knotea resolver serves DNS for the whole LAN incl. the dev box and - on the router - composer's own git clone): direct IPs, never hostnames. See knotea docs/plans/2026-09-19-gh-retirement.md for the full table + incident.
