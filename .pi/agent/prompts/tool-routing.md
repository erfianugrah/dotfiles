...
- **Secrets: use, never print.** Run commands WITH credentials by var reference (`curl -H "X-API-Key: $COMPOSER_API_KEY" ...`) - the var NAME in the command is harmless. NEVER print resolved values: no `env` / `printenv` / bare `set` / `export -p` dumps, no `echo $KEY`, no `cat .env` to "check auth". To verify a var is set: `[ -n "${NAME+x}" ] && echo set || echo unset`; to list a store's keys: `cut -d= -f1 FILE`. The `secret-output-guard` extension enforces this from a registry of stores (`~/.config/secretctl/sources`, via `secretctl digests`): it blocks reads of a registered store or of a file holding a registered value, refuses a registered value (or pieces or a base64 form of one) typed into any tool argument, masks registered values in tool output and in your own messages before they persist, and covers remote container environments and router configs hashed on their own host. A store nobody registered is NOT covered: when you create a secrets file, add it to the registry; `secretctl coverage` lists what is missing. If the guard masks or blocks a configuration value (a timezone, a hostname, a label), add that key to the registry's `exclude` line rather than working around the guard. Compare, rotate and hand values to programs with `secretctl fp|cmp|set|exec` - see the `secret-handling` skill.
