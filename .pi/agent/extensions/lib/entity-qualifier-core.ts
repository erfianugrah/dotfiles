/**
 * entity-qualifier-core - pure detection for "a bare device identifier cited as
 * EVIDENCE must name its host". ZERO harness imports. Source of truth for both
 * the pi adapter (../entity-qualifier-nudge.ts, a message_end nudge) and the
 * Claude Code hook (../../../.claude/hooks/entity-qualifier-nudge.ts, a
 * PreToolUse guard on Write/Edit/MultiEdit).
 *
 ...
 * onboard 1G Realtek on a bridge; the 10G path under discussion is a different
 * card entirely. The cited fact was real, the argument built from it was
 * fabricated, and the record it came from HAD the qualifiers - they were
 * dropped in the retelling.
 *
 * This cannot check whether the right entity was picked; that is the reasoning
 * being policed, and no mechanical check can do that. What it CAN do is force
 * the disambiguation: writing "servarr's eth0" instead of bare "eth0" is the
 * moment a cross-host mix-up becomes visible to a human reader.
 *
 * The one precise trap: a DATE sitting next to the identifier looks like it
 * qualifies something ("on 2026-08-08") but does not - a date says WHEN, not
 * WHICH BOX. So "on/at/for <token>" only counts as a qualifier when `<token>`
 * is a name, never a date or a generic determiner.
 *
 * Only node stdlib + the zero-harness sentences.ts helper are imported.
 */
...
export function hasDeviceId(text: string): boolean {
 ...
 */
export function hasHostQualifier(text: string): boolean {
  ...
  // The stopword list is what keeps `we saw eth0` from reading "saw" as a
  // hostname; it is a heuristic, and an unusual verb directly before an
  // interface name will slip through as a false qualifier.
...
export function answerText(msg: unknown): string {
