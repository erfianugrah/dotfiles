/**
 * video-review-core - pure conversation-analysis logic + a harness-agnostic
 * orchestrator over the local whisper stack (`:7860`). ZERO harness imports
 * (node stdlib + global fetch only). Source of truth for the pi adapter
 * (../video-review.ts) and the Claude Code MCP toolkit
 * (../../../.claude/mcp/toolkit.ts).
 *
 * The pure math (utterance merging, overlap, metrics, question-flow,
 * name-suggestion, name-map parsing) and the whisper HTTP orchestration both
 * live here. The bundle cache is on-disk and harness-agnostic, so it lives
 * here too. Each harness adapter is just a tool-registration shim + progress
 * plumbing.
 *
 * Extracted from video-review.ts (2026-08-12); see
...
// ── HTTP helpers (side-effectful, harness-agnostic) ───────────────────────
...
export function extractOpts(p: ExtractParams) {
  return {
...
export type VideoAction = "extract" | "overlap" | "metrics" | "doc" | "enroll" | "name";

export interface VideoReviewArgs {
  action: VideoAction;
  // shared
  file?: string;
  bundle?: string;
  ...
  output_path?: string;
  // enroll / name
  enroll_action?: "add" | "list" | "remove";
  name?: string;
  map?: string;
}

/**
 * Single harness-agnostic entry point. `onUpdate` streams progress strings;
 * `signal` cancels long transcriptions. Returns { text, details, isError }.
 * Errors are caught and returned as { isError:true } so the harness renders
 * them uniformly rather than throwing across the tool boundary.
 */
export async function runVideoReview(
  args: VideoReviewArgs,
  onUpdate?: (m: string) => void,
  signal?: AbortSignal,
): Promise<VideoResult> {
  try {
    switch (args.action) {
      case "extract":
        return await doExtract(args, onUpdate, signal);
      case "overlap":
        return await doOverlap(args, onUpdate, signal);
      case "metrics":
        return await doMetrics(args, onUpdate, signal);
      case "doc":
        return await doDoc(args, onUpdate, signal);
      case "enroll":
        return await doEnroll(args, signal);
      case "name":
        return await doName(args, signal);
      default:
        return { isError: true, text: `unknown action "${(args as { action?: string }).action}"`, details: { error: "bad-action" } };
    }
  } catch (e) {
    return { isError: true, text: e instanceof Error ? e.message : String(e), details: { error: "exception" } };
  }
}

async function doExtract(args: VideoReviewArgs, onUpdate: ((m: string) => void) | undefined, signal: AbortSignal | undefined): Promise<VideoResult> {
  if (!args.file) return { isError: true, text: "extract requires `file`", details: {} };
  const opts = extractOpts(args);
  const { bundle, path, fromCache } = await ensureBundle(args.file, opts, onUpdate, signal);
  ...
  if (bundle.names && Object.keys(bundle.names).length) {
    ...
    lines.push(`names: none matched - enroll with action:enroll, or label via action:name`);
  }
  const suggestions = bundle.hasWordSpeakers ? suggestNames(bundle) : [];
  if (suggestions.length) {
    lines.push("name suggestions (verify against the transcript, apply via action:name):");
  ...
  lines.push("Next: action:overlap for conversation review, action:doc for meeting-notes source.");
  if (utts.length) {
    ...
    if (!external && !bundle.params?.format) {
      ...
      if (guess) lines.push(`format: untagged - heuristic suggests "${guess}" (tag via action:metrics format=${guess} to build a baseline)`);
    }
  }
  return { text: lines.join("\n"), details: { bundle: path, speakers: bundle.speakers, duration: bundle.duration, fromCache } };
}

async function loadBundle(args: VideoReviewArgs, onUpdate: ((m: string) => void) | undefined, signal: AbortSignal | undefined, extractFrames = false): Promise<{ bundle: Bundle | null; key: string | null }> {
  let bundle: Bundle | null = null;
  let key: string | null = null;
  if (args.bundle) {
    bundle = readBundleByPath(args.bundle);
    key = bundleKeyFromPath(args.bundle);
  }
  if (!bundle && args.file) {
    const opts = extractOpts({ diarize: true, frames: extractFrames });
    const eb = await ensureBundle(args.file, opts, onUpdate, signal);
    bundle = eb.bundle;
    key = eb.key;
  }
  return { bundle, key };
...
async function doOverlap(args: VideoReviewArgs, onUpdate: ((m: string) => void) | undefined, signal: AbortSignal | undefined): Promise<VideoResult> {
  const { bundle } = await loadBundle(args, onUpdate, signal);
  if (!bundle) return { isError: true, text: "provide `bundle` (from action:extract) or `file`", details: {} };
  if (!bundle.hasWordSpeakers) return { isError: true, text: "bundle has no word-level speaker timing; re-run action:extract with diarize:true", details: {} };

  const minOv = args.min_overlap_sec ?? 0.3;
  const maxEv = args.max_events ?? 40;
...
async function doMetrics(args: VideoReviewArgs, onUpdate: ((m: string) => void) | undefined, signal: AbortSignal | undefined): Promise<VideoResult> {
  const { bundle, key: bundleKey } = await loadBundle(args, onUpdate, signal);
  if (!bundle) return { isError: true, text: "provide `bundle` (from action:extract) or `file`", details: {} };
  if (!bundle.hasWordSpeakers) return { isError: true, text: "bundle has no word-level speaker timing; re-run action:extract with diarize:true", details: {} };

  if (args.format && bundleKey && bundle.params?.format !== args.format) {
    bundle.params = { ...(bundle.params ?? {}), format: args.format };
    writeBundle(bundleKey, bundle);
  }
  const callFormat = args.format ?? (bundle.params?.format as string | undefined) ?? null;

  const report = computeMetrics(mergeUtterances(bundle.segments), args.min_overlap_sec ?? 0.3);
  const events = args.question_window_sec != null ? analyzeQuestions(mergeUtterances(bundle.segments), args.question_window_sec) : report.questionEvents;
  const maxEv = args.max_events ?? 30;
...
async function doDoc(args: VideoReviewArgs, onUpdate: ((m: string) => void) | undefined, signal: AbortSignal | undefined): Promise<VideoResult> {
  const { bundle } = await loadBundle(args, onUpdate, signal, args.include_frames ?? false);
  if (!bundle) return { isError: true, text: "provide `bundle` (from action:extract) or `file`", details: {} };

  const wantTranscript = args.include_transcript ?? true;
  const wantFrames = (args.include_frames ?? true) && !!bundle.frames?.length;
  const wantOverlap = (args.include_overlap ?? true) && bundle.hasWordSpeakers;
  ...
  if (wantTranscript) {
    const speakerFilter = args.speaker ? new Set(args.speaker.split(",").map((s) => s.trim()).filter(Boolean)) : null;
    const startS = args.start ?? -Infinity;
    const endS = args.end ?? Infinity;
    const tsegs = bundle.segments.filter(
      (s) => s.end >= startS && s.start <= endS && (!speakerFilter || (s.speaker != null && speakerFilter.has(s.speaker))),
    );
    md.push(`## Transcript`);
    if (speakerFilter || args.start != null || args.end != null) {
      md.push(`(filtered${speakerFilter ? ` speakers: ${[...speakerFilter].join(", ")}` : ""}${args.start != null ? ` from ${hhmmss(args.start)}` : ""}${args.end != null ? ` to ${hhmmss(args.end)}` : ""} - ${tsegs.length} of ${bundle.segments.length} segments)`);
    }
  ...
  if (args.output_path) {
    ...
    return { text: stats.join("\n"), details: { file: bundle.file, duration: bundle.duration, speakers: bundle.speakers, output_path: args.output_path } };
  }

  return { text, details: { file: bundle.file, duration: bundle.duration, speakers: bundle.speakers } };
}

async function doEnroll(args: VideoReviewArgs, signal: AbortSignal | undefined): Promise<VideoResult> {
  const action = args.enroll_action ?? "add";
  if (action === "list") {
    ...
    return { text: `enrolled voice prints:\n${text}`, details: { voiceprints: vps } };
  }
  if (action === "remove") {
    if (!args.name) return { isError: true, text: "remove requires `name`", details: {} };
    const r = await fetch(`${WHISPER_URL}/api/voiceprints/${encodeURIComponent(args.name)}`, { method: "DELETE", signal });
    if (!r.ok) return { isError: true, text: `remove failed: HTTP ${r.status}`, details: {} };
    return { text: `removed voice print: ${args.name}`, details: { removed: args.name } };
  }
  // add
  if (!args.name) return { isError: true, text: "add requires `name`", details: {} };
  let body: Record<string, unknown>;
  if (args.bundle) {
    const b = readBundleByPath(args.bundle);
    if (!b) return { isError: true, text: `bundle not found: ${args.bundle}`, details: {} };
    if (!args.speaker) return { isError: true, text: "add-from-bundle requires `speaker` (a diarized label or display name)", details: {} };
    const hit = resolveEmbedding(b, args.speaker);
    if (!hit) {
      ...
      return { isError: true, text: `no embedding for speaker "${args.speaker}". Available: ${avail}`, details: {} };
    }
    body = { name: args.name, embedding: hit.vec, source: `${b.file}#${hit.label}` };
  } else if (args.file) {
    const serverPath = await resolveToServerPath(args.file, signal);
    body = { name: args.name, file_path: serverPath, start: args.start, end: args.end };
  } else {
    return { isError: true, text: "add requires either `bundle`+`speaker` or `file`", details: {} };
  }
  const res = await whisperPostJson<{ name: string; count: number; dim?: number }>("/api/voiceprints", body, signal);
  return {
    text: `enrolled "${res.name}" (now ${res.count} reference print${res.count === 1 ? "" : "s"}). Future diarized transcripts will auto-name this speaker.`,
    details: res as unknown as Record<string, unknown>,
  };
}

async function doName(args: VideoReviewArgs, signal: AbortSignal | undefined): Promise<VideoResult> {
  if (!args.bundle) return { isError: true, text: "name requires `bundle`", details: {} };
  if (!args.map) return { isError: true, text: "name requires `map`", details: {} };
  const b = readBundleByPath(args.bundle);
  if (!b) return { isError: true, text: `bundle not found: ${args.bundle}`, details: {} };
  const map = parseNameMap(args.map);
  if (!Object.keys(map).length) return { isError: true, text: "empty/unparseable map - use JSON or 'LABEL=Name, ...'", details: {} };
  const before = [...b.speakers];
  applyNameMap(b, map);
  const key = bundleKeyFromPath(args.bundle);
  writeBundle(key, b);
  const lines = [`relabeled: ${Object.entries(map).map(([k, v]) => `${k} -> ${v}`).join(", ")}`, `speakers now: ${b.speakers.join(", ")}`, `(was: ${before.join(", ")})`];
  if (!args.enroll_action && b.speakerEmbeddings && Object.keys(b.speakerEmbeddings).length) {
    lines.push("tip: pass enroll_action:add to also store these as server-side voice prints (auto-named in future transcripts).");
  }
  if (args.enroll_action === "add") {
