...
const applyPatchTool = defineTool({
  ...
  label: "Apply Patch",
  promptSnippet:
    "apply_patch — multi-file diff envelope. Add/Update/Delete files in one atomic call. Alternative to repeated `edit` calls.",
  promptGuidelines: [
    "Use apply_patch when making coordinated changes to 3+ files in one logical commit. For 1-2 files, prefer `edit`.",
    "Patch is atomic — if any hunk fails to apply (e.g. old block not found), NO changes are written.",
    "Each Update hunk must match the file's current content EXACTLY in the - lines. No fuzzy matching.",
  ],
  description: [
    "Apply a multi-file patch in a single atomic operation. Supports Add, Update, and Delete operations within a `*** Begin Patch` / `*** End Patch` envelope.",
    "",
    "Envelope format:",
    "",
    "```",
    ...
    "",
    "Rules:",
    "- Add File: every line MUST start with '+', joined as the new file body.",
    "- Delete File: removes the file. No body.",
    "- Update File: one or more @@-prefixed hunks. Each - line must match the file exactly; + lines replace them. Old block must be UNIQUE in the file (use @@ context to disambiguate if needed).",
    "",
    "Atomic: if any hunk fails, no changes are written. Errors include the failing hunk index and path for fast debugging.",
    "",
    "When to use vs `edit`: `edit` is simpler for single-file changes. `apply_patch` is preferable when 3+ files change together (cross-file refactors, coordinated additions, etc.).",
  ].join("\n"),
