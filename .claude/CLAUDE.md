...
"It will work after a restart" is an untested claim, not a caveat. Deferring verification to a future session is how dead config ships: the 2026-08-27 incident shipped a guard that was written, unit-tested, committed and documented but never stow-linked, so no restart could have loaded it. Config that loads at process start is testable NOW in a fresh process (`claude -p '<prompt that should trip it>' --allowedTools Write` for a hook, `pi -p '...'` for a pi extension), and install-path changes get verified at the LIVE path (`stat -c '%N'` on the symlink; `stow-drift` exits 1 on `UNLINKED`) rather than in the repo. If a spawned-process check is truly impossible, report it as a KNOWN GAP, not as completion.
